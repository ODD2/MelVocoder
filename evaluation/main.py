    ...
    with torch.no_grad():
        ...
        for wav_path in iterator:
            ...
            if _F0:
                ...
                _, frequency_true, confidence, _ = crepe.predict(
                    y_16k, 16000, viterbi=True, verbose=0, model_capacity='medium')
                _, frequency_pred, _, _ = crepe.predict(
                    y_g_hat_16k, 16000, viterbi=True, verbose=0, model_capacity='medium')
                filtered_data = [(true, pred) for true, pred, conf in zip(
                    frequency_true, frequency_pred, confidence) if conf > 0.6 and 50 < true < 2100]
...
def main():
    ...
    gt_dir = './dataset/audio/valid'
    synth_dir = './inference_results/' + a.model
